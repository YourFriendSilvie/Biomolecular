...
public class VoxelRaycastDebugTool : MonoBehaviour
{
    ...
    [SerializeField] private bool showTerrainProfileOverlay = false;
    ...
    private void UpdateOverlaySummary(bool overlayEnabled)
    {
        ...
        if (showTerrainProfileOverlay && voxelTerrain != null && voxelTerrain.HasGeneratedTerrain)
        {
            overlaySummary += "\n" + voxelTerrain.GetTerrainProfileDebugInfo(worldPoint);
        }
    }
    ...
    private void OnGUI()
    {
        ...
        paletteRect.height = paletteCollapsed ? 56f : (showTerrainProfileOverlay ? 470f : 340f);
    ...
    private void DrawPaletteWindow(int windowId)
    {
        ...
        if (!paletteCollapsed)
        {
            ...
            GUILayout.BeginHorizontal();
            if (GUILayout.Button("Log Profile"))
            {
                if (TryGetPrimaryRaycastPoint(out Vector3 profilePoint) && voxelTerrain != null)
                    ReportAction(voxelTerrain.GetTerrainProfileDebugInfo(profilePoint));
                else
                    ReportAction("Log Profile: no terrain hit.");
            }

            showTerrainProfileOverlay = GUILayout.Toggle(showTerrainProfileOverlay, "Profile Overlay");
            GUILayout.EndHorizontal();
            ...
            if (showOverlay)
            {
                ...
                overlayScroll = GUILayout.BeginScrollView(overlayScroll, GUILayout.Height(showTerrainProfileOverlay ? 240f : 120f));
