...
public class ProceduralVoxelTerrainWaterSystem : MonoBehaviour, IRaycastHarvestableProvider
{
    ...
    [SerializeField, Min(0f), Tooltip("Lakes to generate per 16×16 chunk region. Scales automatically with terrain size.")]
    private float lakeDensityPer16x16Chunks = 0.8f;
    ...
    [SerializeField, Min(0f), Tooltip("Ponds to generate per 16×16 chunk region. Scales automatically with terrain size.")]
    private float pondDensityPer16x16Chunks = 2.0f;
        ...
        try
        {
            ...
            else
            {
                ...
                if (ShouldGenerateLakes())
                {
                    ...
                    int lakeCount = ComputeFreshwaterCount(lakeDensityPer16x16Chunks, resolvedTerrain, bounds);
                ...
                if (ShouldGeneratePonds())
                {
                    ...
                    System.Diagnostics.Stopwatch stopwatch = System.Diagnostics.Stopwatch.StartNew();
                    int pondCount = ComputeFreshwaterCount(pondDensityPer16x16Chunks, resolvedTerrain, bounds);
                ...
                // All lake/pond basins have been carved.  Rebuild the profile-texture lake
                // surface Y channel once, covering the full basin area (radius + 1 voxel)
                // so wall columns at the carved edge also receive the correct lakeSurfaceY.
                // This replaces the old per-lake RefreshProfileTextureForWaterBody patches.
                if (ShouldGenerateLakes() || ShouldGeneratePonds())
                {
                    resolvedTerrain.RebuildLakeProfileData();
                }
    ...
    private void ClearGeneratedWaterContents()
    {
        ...
        // Reset stored lake basins and re-upload profile texture channel B so the shader
        // no longer shows basin colours for lakes that no longer exist.
        if (voxelTerrain != null)
        {
            voxelTerrain.ClearRegisteredLakeBasins();
            voxelTerrain.RebuildLakeProfileData();
        }
    ...
    {
        return generateFreshwater && generateLakes && lakeDensityPer16x16Chunks > 0f;
    }

    private bool ShouldGeneratePonds()
    {
        return generateFreshwater && generatePonds && pondDensityPer16x16Chunks > 0f;
    }
    ...
    private static int ComputeFreshwaterCount(float densityPerRegion, ProceduralVoxelTerrain terrain, Bounds bounds)
    {
        ...
        return Mathf.Max(1, Mathf.RoundToInt(densityPerRegion * terrainArea / referenceRegionArea));
    }
